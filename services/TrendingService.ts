...
const calculateTrendingScrore = (post: IPost): number => { //TODO: calculateTrendingScrore -> calculateTrendingScore
	...
	if (post?.interactions) {
		interactionScore += post.interactions.clicked *
			EVENT_TYPE_STRENGTH.IS_CLICKED;
		interactionScore += post.interactions.profileClicked *
			EVENT_TYPE_STRENGTH.IS_PROFILE_CLICKED;
		interactionScore += post.interactions.likes *
			EVENT_TYPE_STRENGTH.IS_LIKED;
		interactionScore += post.interactions.comments *
			EVENT_TYPE_STRENGTH.IS_COMMENTED;
		interactionScore += post.interactions.shares *
			EVENT_TYPE_STRENGTH.IS_SHARED;
		interactionScore += post.interactions.bookmarked *
			EVENT_TYPE_STRENGTH.IS_BOOKMARKED;
		interactionScore += post.interactions.photoExpanded *
			EVENT_TYPE_STRENGTH.IS_PHOTO_EXPANDED;
		interactionScore += post.interactions.videoPlayback *
			EVENT_TYPE_STRENGTH.IS_VIDEO_PLAYBACK;
	}

	const trendingScore = interactionScore *
		Math.exp(-DECAY_RATE * timeDifference);
