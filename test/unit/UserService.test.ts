...
import { beforeEach, afterEach } from '../../deps.ts';
import { CreateUserRequest, assertEquals, assertObjectMatch, describe, it, } from '../../deps.ts';
...
describe("Unit Test - User service", () => {
    ...
    describe("Create user", () => {
        ...
        it("should return an object of failed if password not satisfy minimum 8 characters, at least one letter and one number", async () => {
            ...
            const expected = {
                success: false,
                message: "Password required minimum eight characters, at least one letter and one number"
            }
            assertObjectMatch(actualRes, expected);
        });
        ...
        it("should return an object of failed if user already exist", async () => {
            ...
            const userId = await UserCollection.findOne({ username: request.username })
